...
import club.dawdler.fatjar.loader.file.RandomAccessData;
...
public class JarFileEntries implements CentralDirectoryVisitor, Iterable<NestedJarEntry> {
	...
	private final Map<Integer, FileHeader> entriesCache = new LinkedHashMap<>(16, 0.75f, true) {
	...
	private <T extends FileHeader> T getEntry(CharSequence name, Class<T> type, boolean cacheEntry) {
		...
		if (!isMetaInfEntry(name) && isMultiReleaseJar()) {
			...
			AsciiBytes nameAlias = (entry instanceof NestedJarEntry jarEntry) ? jarEntry.getAsciiBytesName()
