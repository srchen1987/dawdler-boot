        ...
        try {
            ...
            if (sslHostConfig.getGroupList() != null) {
                ...
                try (var localArena = Arena.ofConfined()) {
                    if (SSL_CTX_set1_groups_list(sslCtx, localArena.allocateUtf8String(sb.toString())) <= 0) {
        ...
        for (OpenSSLConfCmd command : conf.getCommands()) {
            ...
            try (var localArena = Arena.ofConfined()) {
                if (name.equals(OpenSSLConfCmd.NO_OCSP_CHECK)) {
                    ...
                    rc = SSL_CONF_cmd(state.confCtx, localArena.allocateUtf8String(name), localArena.allocateUtf8String(value));
                    String errorMessage = OpenSSLLibrary.getLastError();
    ...
    public void init(KeyManager[] kms, TrustManager[] tms, SecureRandom sr) throws KeyManagementException {
        ...
        try (var localArena = Arena.ofConfined()) {
            ...
            if (minTlsVersion <= TLS1_2_VERSION()) {
                if (SSL_CTX_set_cipher_list(state.sslCtx, localArena.allocateUtf8String(sslHostConfig.getCiphers())) <= 0) {
            ...
            if (maxTlsVersion >= TLS1_3_VERSION()) {
                try {
                    if (SSL_CTX_set_ciphersuites(state.sslCtx, localArena.allocateUtf8String(sslHostConfig.getCipherSuites())) <= 0) {
            ...
            if (tms != null) {
                ...
                MemorySegment caCertificateFileNative = sslHostConfig.getCaCertificateFile() != null ?
                        localArena
                                .allocateUtf8String(SSLHostConfig.adjustRelativePath(sslHostConfig.getCaCertificateFile())) :
                        MemorySegment.NULL;
                MemorySegment caCertificatePathNative = sslHostConfig.getCaCertificatePath() != null ?
                        localArena
                                .allocateUtf8String(SSLHostConfig.adjustRelativePath(sslHostConfig.getCaCertificatePath())) :
                        MemorySegment.NULL;
    ...
    public boolean addCertificate(SSLHostConfigCertificate certificate, Arena localArena) throws Exception {
        ...
        if (certificate.getCertificateFile() != null) {
            ...
            MemorySegment certificateFileBytesNative =
                    localArena.allocateArray(ValueLayout.JAVA_BYTE, certificateFileBytes);
            ...
            try {
                ...
                if (certificate.getCertificateFile().endsWith(".pkcs12")) {
                    ...
                    if (keyPassToUse != null && !keyPassToUse.isEmpty()) {
                        passwordAddress = localArena.allocateUtf8String(keyPassToUse);
                    ...
                    MemorySegment certificateKeyFileBytesNative =
                            localArena.allocateArray(ValueLayout.JAVA_BYTE, certificateKeyFileBytes);
                    ...
                    if (MemorySegment.NULL.equals(key)) {
                        if (!MemorySegment.NULL.equals(OpenSSLLibrary.enginePointer)) {
                            ...
                            key = ENGINE_load_private_key(OpenSSLLibrary.enginePointer,
                                    localArena.allocateUtf8String(SSLHostConfig.adjustRelativePath(certificateKeyFileName)),
                ...
                if (!openssl_h_Compatibility.BORINGSSL) {
                    if (!openssl_h_Compatibility.OPENSSL3) {
                        ...
                        if (!MemorySegment.NULL.equals(ecparams)) {
                            ...
                            var curveNidAddress = localArena.allocate(ValueLayout.JAVA_INT, curveNid);
                ...
                if (certificate.getCertificateChainFile() != null) {
                    ...
                    MemorySegment certificateChainBytesNative =
                            localArena.allocateArray(ValueLayout.JAVA_BYTE, certificateChainBytes);
                ...
                if (sslHostConfig.getCertificateRevocationListFile() != null) {
                    ...
                    var certificateRevocationListFileNative = localArena.allocateUtf8String(
                ...
                if (sslHostConfig.getCertificateRevocationListPath() != null) {
                    ...
                    var certificateRevocationListPathNative = localArena.allocateUtf8String(
            ...
            var rawKey = localArena.allocateArray(ValueLayout.JAVA_BYTE, encodedKey.getBytes(StandardCharsets.US_ASCII));
