        ...
        synchronized (lock) {
            ...
            try (var memorySession = Arena.ofConfined()) {
                    ...
                    if ("auto".equals(engineName)) {
                        ...
                        if (MemorySegment.NULL.equals(enginePointer)) {
                            ...
                            if (enginePointer != null) {
                                if (ENGINE_ctrl_cmd_string(enginePointer, memorySession.allocateUtf8String("SO_PATH"), engine,
                                        0) == 0 ||
                ...
                if (openssl_h_Compatibility.OPENSSL3 || !(null == FIPSMode || "off".equalsIgnoreCase(FIPSMode))) {
                    ...
                    if (openssl_h_Compatibility.OPENSSL3) {
                        var md = EVP_MD_fetch(MemorySegment.NULL, memorySession.allocateUtf8String("SHA-512"),
                                MemorySegment.NULL);
        ...
        if (error != SSL_ERROR_NONE()) {
            try (var localArena = Arena.ofConfined()) {
                do {
                    ...
                    var buf = localArena.allocate(ValueLayout.JAVA_INT, OPENSSL_ERROR_MESSAGE_BUFFER_SIZE);
